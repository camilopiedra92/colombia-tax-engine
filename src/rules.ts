...
export function getTaxRules(year: TaxYear) {
  ...
  return {
    ...
    GENERAL: {
      ...
      LIMIT_40_PCT: 0.4, // 40% del ingreso neto
      ...
      AFC_FPV_PCT: 0.3, // 30% del ingreso
      ...
      // GMF (Art. 115 ET)
      GMF_DEDUCTIBLE_PCT: 0.5, // 50% del 4x1000 pagado
      ...
      INFLATIONARY_COMPONENT_PCT:
        year === 2024
          ? 0.5088
          : year === 2025
            ? 0.5088 // Asumido igual hasta nueva publicación
            : 0.5088,
    },
    ...
    DIVIDENDOS: {
      ...
      // No residentes
      NON_RESIDENT_RATE: 0.2, // 20% para no residentes
    ...
    GANANCIA_OCASIONAL: {
      ...
      RATE_LOTTERIES: 0.2, // 20% loterías, rifas, premios (Art. 317)
      ...
      EXEMPT_OTHER_BENEFICIARY_PCT: 0.2, // 20% para beneficiarios no legitimarios (Art. 307 num 3)
    ...
    DESCUENTOS: {
      ...
      RD_INVESTMENT_PCT: 0.3, // 30% inversión I+D
    ...
    ANTICIPO: {
      ...
      SECOND_YEAR_PCT: 0.5, // 50% segundo año (Art. 807 inc. 2)
    ...
    IMPUESTO_PATRIMONIO: {
      ...
      TABLE: [
        ...
        { min: 122000, max: 239000, rate: 0.01, baseTax: 250 }, // 250 UVT + 1% sobre exceso 122,000
...
export function calculateSocialSecurity(ibcBruto: number, smlmv: number, isIndependent: boolean) {
  ...
  const baseIBC = isIndependent ? ibcBruto * 0.4 : ibcBruto;
  ...
  if (smlmvCount >= 4) {
    ...
    else if (smlmvCount >= 20) fspRate += 0.01; // Max 2% total
  }
